...
description: 当用户想在 Windows 上采集网易云音乐桌面版听歌画像数据时使用。此 Skill 通过 CDP 启动或连接 `cloudmusic.exe`，在已登录页面上下文用网易云 `/api` 采集用户创建歌单、主歌单、最近一周听歌排行和所有时间听歌排行，写出 raw/result/csv/aggregate/log 文件，并给出两版分析 prompt；本 Skill 不在采集会话内做人格或画像分析。
---
...
脚本需要 `Python 3.10+`、`psutil`、`requests`、`websocket-client`；缺依赖时执行 `python -m pip install -r scripts/requirements.txt`。
...
3. 如果检测到 NetEase Cloud Music 已经打开，立即停止，不要采集、不要分析、不要自动复用旧数据。展示检测到的 PID 和 exe 路径，方便用户确认确实有进程存在，然后询问用户：
...
- 只支持 Windows Win32 桌面版 `cloudmusic.exe`。
- 固定使用 CDP 端口 `9222`，不要尝试其他端口。
- 如果 NetEase Cloud Music 已经运行，要求用户手动关闭；不要 kill 进程。
- 如果 `cloudmusic.exe` 找不到或找到多个候选路径，询问用户准确路径并传入 `--cloudmusic-exe`。
